<template>
  <div class="max-w-6xl mx-auto min-h-screen py-20">
    <div class="flex justify-between items-center mb-8">
      <NuxtLink class="bg-[#FF8911] text-white py-3 px-4 font-ibm-plex-sans font-semibold rounded-md" to="/user/create">
        Add
        User
      </NuxtLink>
      <div class="flex items-center gap-4">
        <label for="display">Display</label>
        <select id="display" v-model="users.per_page"
    ...
    <div class="border border-slate-200 rounded-lg shadow">
      <table class="table-fixed w-full border-collapse">
        <thead class="bg-slate-50 border-b">
          <tr>
            <th class="font-semibold font-raleway p-5">
              Name
            </th>
            <th class="font-semibold font-raleway p-5">
              Email
          ...
          <tr>
            <th class="p-4 pt-0">
              <input placeholder="Search Name.." v-model="users.search.name"
                class="outline-[#7d27ffe0] rounded-md py-2 px-3 border border-gray-300 font-medium font-ibm-plex-sans"
                type="text">
            </th>
            <th class="p-4 pt-0">
              <input placeholder="Search Email.." v-model="users.search.email"
                class="outline-[#7d27ffe0] rounded-md py-2 px-3 border border-gray-300 font-medium font-ibm-plex-sans"
                type="text">
            </th>
            <th class="p-4 pt-0">
              <select v-model="users.search.gender"
                class="outline-[#7d27ffe0] rounded-md py-2 px-3 border border-gray-300 font-medium font-ibm-plex-sans"
                placeholder="Lokasi">
            ...
            <th class="p-4 pt-0">
              <select v-model="users.search.status"
                class="outline-[#7d27ffe0] rounded-md py-2 px-3 border border-gray-300 font-medium font-ibm-plex-sans"
                placeholder="Lokasi">
          ...
          <tr v-if="!users.loading" class="hover:bg-slate-50" v-for="user in users.data">
            ...
            <td class="p-5">
              ...
              <button data-modal-target="popup-modal" data-modal-toggle="popup-modal" type="button"
                @click="deleteUser(user.id)" class="pl-4 text-red-500">Hapus</button>
            </td>
          </tr>
        </tbody>
      </table>
      <div v-show="users.loading" class="h-72 flex justify-center items-center">
        <svg class="animate-spin -ml-1 mr-3 h-10 w-10 text-[#7d27ffe0]" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
      ...
      <div v-show="users.data" class="flex justify-center py-8 border-t">
        <nav class="relative z-0 inline-flex rounded-md shadow-sm" aria-label="Pagination">
          <button type="button" @click="users.page--" :class="{
            'bg-gray-100':
              users.page == 1 || users.loading,
  ...
  " class="cursor-normal relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 border border-gray-300 rounded-l-md hover:bg-gray-50">
          ...
          <button type="button" @click="users.page = 1" :class="{
            'border-[#7d27ffe0]':
              users.page == 1,
          }"
          ...
          <template v-if="users.page <= 2 ||
            users.page >= users.totalPages - 1
            ">
            <button type="button" @click="users.page = 2" :class="{
              'border-[#7d27ffe0]':
                users.page == 2,
            }"
              class="bg-white text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 text-sm font-medium border">
            ...
            <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border">
              ...
            </span>
    ...
    'border-[#7d27ffe0]':
      ...
      users.totalPages - 1,
          ...
          <template v-else>
            ...
            <button type="button" @click="users.page = users.page - 1" :class="{
              'border-[#7d27ffe0]':
            ...
            <button type="button" @click="users.page = users.page" :class="{
              'border-[#7d27ffe0]':
          ...
          <button type="button" @click="users.page = users.totalPages" :disabled="users.loading" :class="{
            'border-[#7d27ffe0]':
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow">
        ...
        <div class="p-4 md:p-5 text-center">
          ...
          <button @click="openModal(false)" data-modal-hide="popup-modal" type="button"
            class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-100">No,
            cancel</button>
        </div>
      </div>
    </div>
...
async function fetchUsers() {
  ...
  users.value.page = response.headers.get(
    "X-Pagination-Page"
  )
  users.value.totalPages = response.headers.get(
    "X-Pagination-Pages"
  )
